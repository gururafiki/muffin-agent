...
- **`--allow-blocking` is no longer needed** (removed 2026-08-08). The launch config used to pass it because resolving the sandbox backend did a blocking `socket.connect`, which `langgraph dev` refuses under ASGI. The real cost was worse than the flag suggested: deepagents' `FilesystemMiddleware` resolves the backend on *every model call*, so that lookup ran per call even when the model never touched a file. [`langchain-opensandbox`](https://github.com/gururafiki/langchain-opensandbox) now returns a lazy backend that connects on first use and serves the async operations from the async client. If you re-add an eager backend factory, the flag comes back — and so does a network round-trip per model call.
