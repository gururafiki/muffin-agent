...
async def test_council_runs_all_13_personas_to_judge(config, store):
    ...
    # No persona's internal scratch (the ``output=False`` collect_data fields,
    ...
    #
    # This set used to also permit ``tool_runs`` + ``subagent_tree``. Each
    # persona's tool calls and sub-agents now come from that persona's own
    # LangGraph namespace, so if either name reappears here, observability has
    # been put back into graph state — the thing that forced every
    # ``output_schema`` boundary to re-declare three channels with the exact
    # reducer or silently drop a whole subtree.
    allowed = {
        "ticker",
        "query",
        "as_of_date",
