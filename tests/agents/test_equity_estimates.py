"""Tests for the equity estimates data collection agent."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest

from muffin_agent.agents.data_collection.equity_estimates import MCP_TOOLS
...
class TestMCPTools:
    ...
    def test_mcp_tools_all_start_with_equity_estimates(self):
        for tool_name in MCP_TOOLS:
            assert tool_name.startswith("equity_estimates_"), (
...
class TestGetTools:
    ...
    async def test_filters_to_allowed_tools(self):
        ...
        mock_tool_allowed.name = "equity_estimates_consensus"
        ...
        assert len(tools) == 1
        assert tools[0].name == "equity_estimates_consensus"
