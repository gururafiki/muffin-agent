"""Tests for the news data collection agent."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest

from muffin_agent.agents.data_collection.news import MCP_TOOLS
...
class TestMCPTools:
    ...
    def test_mcp_tools_all_start_with_news(self):
        for tool_name in MCP_TOOLS:
            assert tool_name.startswith("news_"), f"Unexpected tool: {tool_name}"
...
class TestGetTools:
    ...
    async def test_filters_to_allowed_tools(self):
        ...
        mock_tool_allowed.name = "news_company"
        ...
        assert len(tools) == 1
        assert tools[0].name == "news_company"
...
class TestPromptTemplate:
    ...
    def test_news_template_renders(self):
        result = render_template("data_collection/news.jinja")
