...
from muffin_agent.agents.portfolio import (
...
from muffin_agent.portfolio.executor import PortfolioOrder
...
class TestPositionSizingNode:
    async def test_empty_tickers_returns_empty(self):
        result = await position_sizing_node({}, {})
...
class TestTickerDecisionNode:
    ...
    async def test_llm_called_when_signals_present(self):
        ...
        with patch(
            "muffin_agent.agents.portfolio.ticker_decision.ModelConfiguration.get_chat_model_for_role",
...
class TestPortfolioReconcilerNode:
    ...
    async def test_llm_called_when_capacity_exists(self):
        ...
        mock_llm.ainvoke = AsyncMock(return_value=fake)
        with patch(
            "muffin_agent.agents.portfolio.portfolio_reconciler.ModelConfiguration.get_chat_model_for_role",
            return_value=mock_llm,
        ):
            result = await portfolio_reconciler_node(
                {
