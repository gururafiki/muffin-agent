"""Tests for the options data collection agent."""

from unittest.mock import AsyncMock, MagicMock, patch

import pytest

from muffin_agent.agents.data_collection.options import MCP_TOOLS
...
class TestMCPTools:
    ...
    def test_mcp_tools_all_start_with_derivatives(self):
        for tool_name in MCP_TOOLS:
            assert tool_name.startswith("derivatives_"), f"Unexpected tool: {tool_name}"
...
class TestGetTools:
    ...
    async def test_filters_to_allowed_tools(self):
        ...
        mock_tool_allowed.name = "derivatives_options_chains"
        ...
        assert len(tools) == 1
        assert tools[0].name == "derivatives_options_chains"
