...
``risk_management_node`` fell back to its hardcoded 25% volatility
...
class TestFetchPricesHistory:
    async def _patched_fetch(
        ...
        *,
        ...
        with (
            ...
            patch(
                "muffin_agent.agents.personas_council.portfolio.portfolio_decision.get_store",
                return_value=store,
            ),
        ):
    ...
    async def test_mcp_failure_returns_empty(self):
        ...
        with patch(
            "muffin_agent.agents.personas_council.portfolio.portfolio_decision.get_tools",
...
class TestPerTickerSubgraphPriceFlow:
    ...
    async def test_run_council_populates_prices_history(self):
        ...
        with (
            patch(
                "muffin_agent.agents.personas_council.portfolio.portfolio_decision.build_council_graph",
            ...
            patch(
                "muffin_agent.agents.personas_council.portfolio.ticker_decision.ModelConfiguration."
...
class TestPositionSizingWithRealBars:
    ...
    async def test_real_bars_yield_non_default_vol(self):
        from muffin_agent.agents.personas_council.portfolio import risk_management_node
