...
class TestCreateMarketRegimeAgent:
    ...
    async def test_creates_agent_with_macro_subagents(self):
        ...
        with (
            ...
            # `market_regime` without a node function.
            assert response_format.schema is MarketRegimeNodeOutput
            assert list(MarketRegimeNodeOutput.model_fields) == ["market_regime"]
            assert (
                MarketRegimeNodeOutput.model_fields["market_regime"].annotation
                is MarketRegimeOutput
            )
