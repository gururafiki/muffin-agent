...
class Score:
    ...
    via :class:`muffin_agent.agents.personas.schemas.ScoreDetail`.  Kept as
...
def compute_buffett_3stage_dcf(
    ...
    for a strong moat).  Stage 2: 5 years at half of stage 1 growth (fading
    competitive advantage).  Stage 3: perpetual ``terminal_growth`` (~2.5%,
    long-run GDP).  Applies a ``conservatism_factor`` haircut (default 0.85,
    matching the ai-hedge-fund reference) to the final intrinsic value to
    enforce additional margin of safety on the calculation itself.

    Args:
        owner_earnings: Starting owner earnings (from
            :func:`compute_owner_earnings`).  Must be positive.
...
def compute_damodaran_fcff_dcf(
    ...
    year ``years``.  Terminal value via Gordon Growth at year ``years + 1``.
    Returns a tuple of (intrinsic_value, discount_rate_used) so the persona
    can cite the WACC in its reasoning.
