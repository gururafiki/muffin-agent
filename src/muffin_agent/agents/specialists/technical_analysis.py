"""Technical analysis specialist — compiled deterministic subgraph (v4).
...
2. ``compute_technical_signal`` — pure Python 5-strategy ensemble via
   :mod:`muffin_agent.tools.technicals`. **No LLM call.**

A legacy bridge ``technical_analysis_node`` is preserved so the existing
``SPECIALIST_REGISTRY`` / council graph / CLI keep working until Phase 5
of the refactor rewrites them to call ``build_technical_analysis_agent``
directly.
"""
