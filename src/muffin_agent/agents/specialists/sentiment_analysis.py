"""Sentiment specialist — compiled deterministic subgraph (v4).
...
A legacy bridge ``sentiment_analysis_node`` is preserved so the existing
``SPECIALIST_REGISTRY`` / council graph / CLI keep working until Phase 5
of the refactor rewrites them to call ``build_sentiment_analysis_agent``
directly.
"""
