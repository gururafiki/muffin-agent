...
class CharlieMungerSignal(AnalystSignal[CharlieMungerEvidence]):
...
class CharlieMungerState(AgentState):
    ...
    evidence: Annotated[
        CharlieMungerEvidence | None, OmitFromSchema(input=True, output=False)
    ]
...
async def render_verdict_node(
    ...
    prompt = render_template(
        "personas_council/personas/charlie_munger.jinja",
        ticker=ticker,
...
async def _build_data_collection_agent(config: RunnableConfig) -> CompiledStateGraph:
    ...
    builder = (
        ...
        .with_input_prompt_template(
            "personas_council/personas/charlie_munger_data_collection.jinja"
        )
