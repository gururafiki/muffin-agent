...
class WarrenBuffettEvidence(BaseModel):
    """Buffett-specific precomputed evidence.
...
class WarrenBuffettRawData(BaseModel):
    ...
    metrics_history: list[BuffettMetricsRow] = Field(
        ...
        description=(
            ...
            "oldest → newest (latest snapshot LAST, matching every other series "
            "in this response).  Each row carries ROE, D/E, operating margin, "
    ...
    current_assets_series: list[float | None] = Field(
        default_factory=list,
        description=(
            "Annual total current assets, oldest → newest, "
            "from equity_fundamental_balance. Used for the working-capital "
            "adjustment in owner earnings."
        ),
    )
    current_liabilities_series: list[float | None] = Field(
        default_factory=list,
        description=(
            "Annual total current liabilities, oldest → newest, "
            "from equity_fundamental_balance. Used for the working-capital "
            "adjustment in owner earnings."
        ),
    )
...
def _score_buffett_fundamentals(
    ...
    Total max = 10 (3+3+2+2). Note this uses muffin's *graduated* sub-score
    bands (``score_roe``: >20→3/>15→2/>10→1; ``score_debt_to_equity``:
    <0.3→3/<0.7→2/<1.5→1; etc.) — these are intentionally finer-grained than
    upstream ai-hedge-fund's binary bands (ROE>15→+2, D/E<0.5→+2,
    op-margin>15→+2, current-ratio>1.5→+1), so the per-dimension scores differ
    even though the aggregate scale (10) coincides.
    """
    if latest is None:
...
def _score_buffett_moat(
    ...
    """Score moat dimensions across the metrics history.

    Accepts an **oldest → newest** series (uniform RawData convention) and
    reverses internally so the margin-improvement check treats the newest half
    as "recent".
    ...
    * Performance stability (CV of ROE + margin) > 70% → +1
    ...
    # Reverse to newest-first for the recent-vs-older margin window logic below.
...
def compute_evidence_node(state: WarrenBuffettState) -> dict[str, Any]:
    ...
    Never crosses an LLM boundary — pure Python.
    """
    metrics_history = state.get("metrics_history") or []
    # State may carry plain dicts — coerce to BuffettMetricsRow
    metrics_rows: list[BuffettMetricsRow] = [
        m if isinstance(m, BuffettMetricsRow) else BuffettMetricsRow.model_validate(m)
        for m in metrics_history
    ]

    # metrics_history is oldest → newest (uniform with all other series), so the
    ...
    # Owner earnings + intrinsic value + MoS (ai-hedge-fund parity)
    ...
    revenue_series = state.get("revenue_series") or []
    current_assets_series = state.get("current_assets_series") or []
    current_liabilities_series = state.get("current_liabilities_series") or []

    owner_earnings = compute_buffett_owner_earnings(
        ni_series,
        dna_series,
        capex_series,
        revenue_series,
    ...
    # Derive conservative DCF growth from historical net-income CAGR
    # (upstream: clamp -5%..+15%, ×0.7 haircut; default 3% when insufficient
    # or negative base). Stage 1 caps at 8%; stage 2 = min(stage1×0.5, 4%).
    ni_vals = [v for v in ni_series if v is not None][-5:]
    if len(ni_vals) >= 3 and ni_vals[0] > 0:
        years_span = len(ni_vals) - 1
        hist_growth = (ni_vals[-1] / ni_vals[0]) ** (1 / years_span) - 1
        conservative_growth = max(-0.05, min(hist_growth, 0.15)) * 0.7
    else:
    ...
    intrinsic_value: float | None = (
        compute_buffett_3stage_dcf(
            owner_earnings,
            growth_stage_1=stage1_growth,
            growth_stage_2=stage2_growth,
        )
...
async def render_verdict_node(
    ...
    prompt = render_template(
        "personas_council/personas/warren_buffett.jinja",
        ticker=ticker,
...
async def _build_data_collection_agent(config: RunnableConfig) -> CompiledStateGraph:
    ...
    builder = (
        ...
        .with_input_prompt_template(
            "personas_council/personas/warren_buffett_data_collection.jinja"
        )
