"""Risk manager — deterministic volatility × correlation position budget.

Ports ai-hedge-fund's ``risk_manager.py`` to a LangGraph-native node
(``risk_management_node``): no LLM, pure-Python.  Computes per-ticker dollar
position limits combining:
...
class PositionSizingInputState(TypedDict, total=False):
    """State keys read by ``risk_management_node``.
...
class PositionSizingOutputState(TypedDict, total=False):
    """State keys written by ``risk_management_node``."""
...
async def risk_management_node(
    ...
    try:
        ...
        logger.exception("risk_management_node: invalid portfolio payload")
        return {
            "position_limits": {
