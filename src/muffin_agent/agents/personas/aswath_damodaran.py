...
Academic FCFF DCF + CAPM. See ``warren_buffett.py`` for the v4 ref.
...
def _score_damodaran_relative(
    ...
    if pe < 15:
        return AswathDamodaranRelativeValuation(
            ...
            score=1,
            max_score=1,
            reasoning=f"P/E {pe:.1f} (cheap absolute)",
        )
    return AswathDamodaranRelativeValuation(
        pe_ratio=pe,
        score=0,
        max_score=1,
        reasoning=f"P/E {pe:.1f}",
    )
...
async def render_verdict_node(
    ...
    result = cast(
        ...
        await llm.ainvoke(
            [SystemMessage(prompt), HumanMessage("Render your Damodaran verdict now.")]
        ),
    )
...
async def _build_data_collection_agent(config: RunnableConfig) -> CompiledStateGraph:
    ...
    builder = (
        ...
        .with_runtime_system_prompt_template(
            "personas/aswath_damodaran_data_collection.jinja"
        )
