"""Conservative Risk Debator node."""
...
class ConservativeDebatorInputState(TypedDict, total=False):
    """State keys read by ``conservative_debator_node``."""
...
class ConservativeDebatorOutputState(TypedDict, total=False):
    """State keys written by ``conservative_debator_node``."""

    risk_conservative_responses: Annotated[list[str], operator.add]


async def conservative_debator_node(
    state: ConservativeDebatorInputState, config: RunnableConfig
) -> ConservativeDebatorOutputState:
    """One Conservative Risk Debator turn."""
    ...
    return {"risk_conservative_responses": [str(response.content).strip()]}
