...
- [x] **Sub-agent execution tree — Phase 1 backend capture (2026-07)** — design spec: `docs/superpowers/specs/2026-07-23-subagent-execution-tree-design.md`. New light `subagent_tree` reducer channel (`dict[node_id -> TreeNode]`, `merge_subagent_tree`) emitted by `AgentCaptureMiddleware` for every capturing agent, unconditionally (same rationale as `tool_runs`); `node_id`/`parent_id` derived from the ambient `checkpoint_ns` (pipe-joined `<node>:<uuid>` segments, the trailing `AgentCaptureMiddleware.*` segment stripped — `agent_capture/tree.py`). Heavy per-node detail (full transcript + tool payloads + output) offloads best-effort to the Store at namespace `("subagent_detail", thread_id)` keyed by node id (`agent_capture/detail_store.py`, async `aafter_agent` only) — keeps `thread.values` light (fast reopen, see the muffin-ui M21 reopen-latency fix) while a UI can fetch detail lazily instead. Propagated across every `output_schema`-restricted subgraph boundary that already carries `tool_runs`: the 13 personas (persona State + `<Persona>Output` + `CouncilState`), the criteria worker (`_CriterionWorkerState` declares the channel; `package_evaluation_node` re-homes it onto each `criterion_evaluations[i]["subagent_tree"]` — the same move it already does for `tool_runs` — so the tree rides *inside* each criterion evaluation rather than as its own top-level channel there; a consumer gathers the top-level `subagent_tree` channel PLUS each per-criterion entry, exactly like `tool_runs`; `_CriterionWorkerOutput` itself is unchanged), `TradingDecisionState`, and `ResearchState`. Integration-tested nesting for council + criteria_analysis. **Supersedes** the reopen work's deferred "Track-2 transcript offload" idea (muffin-ui ROADMAP M20/M21 — slim `subagent_runs` by reconstructing transcripts from checkpoint history once the backend checkpointer-latency issue is fixed): the Store offload achieves the same "keep `thread.values` light" goal directly today, without waiting on that backend fix.
  - **Follow-up: post-deploy smoke test.** Confirm on a real deployed run that `subagent_tree` has `"|"`-nested ids (i.e. `checkpoint_ns` is populated on the LangGraph Platform, the historically-fragile `get_config` call site) and that `task`-subagent nodes appear distinctly (validating the `resolve_node_id` collision guard in the wild). If the tree collapses to a single `__root__` node, `checkpoint_ns` isn't reaching the middleware on the Platform.
  - **Follow-up: `has_detail` is optimistic.** It's hardcoded `True` on every `TreeNode` (`build_tree_node` in `tree.py`), but the sync `after_agent` path never offloads to the Store (only async `aafter_agent` does), and any failed async offload leaves no Store entry either. The Phase-2 UI must tolerate a Store miss on an expanded node rather than trusting `has_detail`.
- [ ] **Sub-agent execution tree — Phase 2 (muffin-ui, separate plan)** — recursive drill-down `SubagentTree` component that renders the captured `subagent_tree` topology: rows expand to a trading-decision-level `SubgraphDetail` (transcript + structured output + `ToolRunsPanel`) plus the node's own children, recursing to any depth; heavy detail fetched lazily per expanded node from the `subagent_detail` Store namespace. Mounted uniformly across council / criteria_analysis / trading_decision / research / stock_evaluation, identical live and on history (reads the captured channel either way).
- [ ] **`subagent_detail` Store GC/TTL** — entries under `("subagent_detail", thread_id)` persist indefinitely, like the tool-result cache does today. No pruning/TTL yet; add one (or a thread-deletion hook) once growth is measured against real traffic.
