...
- [ ] **Stage-level custom-event progress protocol** — extend the shipped per-criterion `criterion_evaluated` event into a structured protocol emitted by every stage node across graphs (criteria stages, council personas, trading analysts): `{type: stage_started|stage_completed, stage, detail}`. Lets muffin-ui drive run plans/counters from explicit events instead of regex-matching node names, and replaces the UI-side values-accumulation workarounds. Cross-ref: muffin-ui ROADMAP "Stage-level progress protocol".
