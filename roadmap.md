...
- [x] **Observability moved to LangGraph's own checkpoints; `agent_capture/` deleted (2026-07-27)** — supersedes
  the whole tool-telemetry / capture-consolidation / sub-agent-execution-tree line of work below, which is kept in
  git history only. The three reducer channels (`tool_runs`, `subagent_runs`, `subagent_tree`) and the
  `("subagent_detail", thread_id)` Store offload are gone. `POST /threads/{id}/history` already returns each
  superstep's `tasks[]` with `{id, name, result, checkpoint:{checkpoint_ns}}`, and recursing on that namespace
  yields the child's supersteps, its tasks, and its `values.messages` — the transcript with its tool calls. The
  invariant that makes it work is structural, not telemetric: **a node is drillable iff it is a compiled
  agent/subgraph added via `add_node`** (`tests/integration/test_graph_observability.py`). Rationale + the
  corrected latency diagnosis: `docs/backend-notes/2026-07-27-api-read-latency-mcp-tool-discovery.md` and the
  CORRECTION block in `docs/superpowers/specs/2026-07-23-subagent-execution-tree-design.md`.
  - Consequence for backups: checkpoints are now the ONLY record of what a run did, and the nightly dump still
    excludes them (`muffin-deployment/stack/muffin-db-backup.sh`). Decide before relying on those dumps for run
    history. Pruning the one 1763 MB errored thread (`019f8476`) brings the tables to ~115 MB, which makes
    including them cheap.
  - Not a storage win: `subagent_runs` was 350 kB of 1878 MB. The justification is removing observability-as-state,
    which forced every `output_schema` boundary to re-declare three channels with the exact reducer or silently
    drop a subtree.
- [ ] **Per-tool duration is still missing.** The UI derives each tool call from the transcript (an `AIMessage`
  tool call paired with its `ToolMessage`), which carries status and payloads but no timing — messages simply
  don't record when a call started or how long it took. Adding it means writing timestamps somewhere durable at
  call time (e.g. an `awrap_tool_call` hook stamping the `ToolMessage`'s `additional_kwargs`, which keeps it
  inside the message rather than reviving a side channel). Related: itemising individual `ToolRetryMiddleware`
  attempts rather than only the final outcome.
...
- [x] **Structured-output completion guard (2026-07)** — a prod criteria run failed at `lift_classification` ("produced no classification"): `minimax-m3:cloud` paired a MALFORMED `TickerClassificationNodeOutput` call with `write_todos` in one `AIMessage`, and an upstream langchain bug (`_make_tools_to_model_edge` exit condition 3, present through ≥1.3.13) ended the loop on "a structured-output ToolMessage exists" without checking parse success — the parse-error feedback appended for retry was never shown to the model. Shipped `_StructuredOutputRetryMiddleware`: universal `after_agent` guard (`can_jump_to=["model"]`, wired whenever a response format is set, registered last = runs first) that re-enters the loop so the model reads the feedback and retries **in-loop**; injects a nudge `HumanMessage` when the schema tool was never called; raises `StructuredOutputRetryExhaustedError` (plain `Exception` subclass → node `RetryPolicy` re-runs the node as backstop) after 3 attempts. E2E repro: `tests/integration/test_structured_output_retry.py`.
    - [ ] **Upstream fix** — report/PR to langchain: condition 3 should require `"structured_response" in state` before ending the loop. Drop the local guard's bug-workaround role once the fix ships (the nudge + exhaustion-cap behaviour is worth keeping regardless).
- [x] **Data-collection enforcement loop (2026-07-27)** — `middlewares/data_collection_guard/`, enabled via
  `MuffinAgentBuilder.with_data_collection_guard(max_attempts=2)`. When an agent is about to end having
  executed nothing but plumbing, the guard injects a corrective `HumanMessage` and jumps back to the model,
  bounded by TWO ceilings (transcript nudges + a `data_collection_jumps` counter — the counter is what
  terminates the loop under `ModelCallLimitMiddleware(exit_behavior="end")`, which every persona collector
  uses). After exhaustion it ACCEPTS the answer rather than raising, so the deterministic truthing still
  labels it `data_collected=False` downstream. Verified end-to-end against the real criterion-evaluation
  worker with a model scripted to always single-shot: 3 model calls (initial + exactly 2 retries), loop
  terminated, answer accepted with `confidence` capped at 0.3 and the limitation recorded. Wired on all 21
  collectors (13 personas, 4 ReAct specialists, 4 trading analysts) + the criterion evaluator;
  `tests/agents/test_data_collection_guard_coverage.py` fails if a new one ships unguarded. The same
  middleware records `executed_tools` (the evidence `_reconcile_data_sources` consumes) — one middleware on
  purpose, so enforcement and reconciliation can never disagree about what counts as collected data.
...
- [ ] **Reflection-loop pushback** — after synthesis, run a reflect step that can either re-issue specific criterion evaluations with refinement notes or request additional criteria from a re-run of `valuation_methodology_node` with the synthesis as input context. **Needs a mechanism designed for this graph shape:** criterion evaluations are `Send` workers (Pattern A), not `task` subagents, so the deleted `SubagentRefinementMiddleware` `prior_call_id` protocol never applied here — a re-issue means a second `Send` wave carrying the reflect notes in the payload.
...
- [~] **W8** — `SubagentRefinementMiddleware` + `SubagentRefinementParentMiddleware` shipped a generic refinement protocol (subagents emit `CollectionFindings` with typed `gaps`; findings cache to `/scratch/subagent_runs/<call_id>.json`; parent re-issues with `prior_call_id=<id>`). **REMOVED 2026-08-09 — never wired.** `with_subagent_refinement()` was never called by any agent in its entire git history; the plan's follow-up step (give the data-collection subagents the response format, teach `stock_evaluation.jinja` the protocol) never happened, and the codebase then moved from `task` subagents to compiled-agent-as-node, which the protocol doesn't fit. Its function is now covered by `DataCollectionGuardMiddleware` (gaps/fabrication) and `ToolKnowledgeMiddleware` (provider-unavailable/upstream-error). See CLAUDE.md § middlewares.
...
- [ ] **Resumable-thread subagents** — replace the stateless `task` tool flow with per-subagent persistent LangGraph threads keyed by `(parent_run_id, subagent_name)`. Parent re-issuing a task on the same key resumes the subagent's checkpoint instead of cold-booting. **Deferral premise corrected 2026-08-09:** this used to say "defer until `SubagentRefinementMiddleware`'s scratch-cache protocol is proven insufficient in production traces" — that protocol never ran in production at all (never wired, now deleted), so it never provided the evidence this item was waiting on. If resumable subagents are wanted, judge them on their own merits.
...
- [ ] **Langfuse migration of observability hooks** — when the user migrates from LangSmith to Langfuse, revisit the "no custom LangSmith tags" decision — emit Langfuse-native trace metadata for `lesson_count`, `cache_hit_rate` so dashboards can filter on adaptation events.
...
- [x] **Council rate-limit tuning for Ollama Cloud** — the ~19-way council fan-out overwhelmed Ollama Cloud with concurrent requests (`ResponseError: too many concurrent requests`, 429) on the AMZN run (trace `019f8476-…`). Set `requests_per_second: 0.5` on the ollama profile in the deployed `llm_chain` (deploy workflow heredoc + `config.example.yml`) — a config change, no agent code (the per-profile `_shared_rate_limiter` already honoured it). Effective concurrency ≈ rps × avg_call_latency; tune to the Ollama Cloud tier. NOTE: `InMemoryRateLimiter` bounds request *start* rate, not a hard in-flight cap — if a true concurrency semaphore is later needed, add one in `_build_chat_model`.
...
### Reliability v2 (post-AMZN council trace `019f8476-…`, 2026-07)

A `council` run on AMZN (`include_specialists=true`) hung for ~37 min, burned ~1.7M tokens, and delivered **no verdict** — the ~19-way persona/specialist fan-out flooded Ollama Cloud with concurrent requests, which cascaded. Shipped fixes:

- [x] **Ollama concurrency throttle** — see "Council rate-limit tuning for Ollama Cloud" above (`requests_per_second: 0.5` on the ollama profile).
- [x] **Structured-retry ↔ model-call-limit livelock** — `_StructuredOutputRetryMiddleware` counted attempts only from transcript artifacts (schema `ToolMessage`s + nudges), which advance only when the model runs. Once `ModelCallLimitMiddleware(exit_behavior="end")` spent the model-call budget the model could never run again → the artifact count froze below `max_attempts` and the guard jumped to `model` forever while `ModelCallLimit` bounced it to `end`, one superstep per bounce until the Platform `recursion_limit` (10011) tripped ~10k iterations later. Added an `operator.add` `structured_retry_jumps` state counter that increments on every guard jump regardless of model execution; `raise` fires on `max(transcript_attempts, jumps) >= max_attempts`. Regression: `test_bounds_jumps_when_model_cannot_run`.
- [x] **`nassim_taleb` `str > int` crash** — `prices_1y` / `insider_trades` are `list[dict[str, Any]]` (Pydantic doesn't coerce inner values, unlike `*_series`); a string `close` / `transaction_shares` broke `prev > 0`. Added `_to_float()` at the dict-read sites. Test: `test_nassim_taleb.py`.
- [x] **ben_graham `limit=8` → 422** — its data-collection prompt hardcoded `limit=8` on `equity_fundamental_*`, but the yfinance provider caps those at 5 → HTTP 422 → "tool call limit exceeded" cascade. Set `limit=5`. Guard test: `test_data_collection_prompts_respect_fundamentals_limit_cap` (asserts no persona requests `limit>5`).

Deferred follow-ups:

- [ ] **Per-persona council error isolation (recommended, top priority)** — the council fans out all ~19 personas/specialists from `START` in ONE superstep with no isolation, so a single persona raising (after its `RetryPolicy`) fails the whole superstep and discards every successful signal (the mechanical reason the AMZN run delivered nothing despite 15+ personas succeeding). Wrap each persona node so a failure writes an `error`/`abstain` `persona_signals` entry instead of raising; `council_judge` already handles dissent/low-confidence and could synthesize a partial verdict.
- [ ] **OpenRouter fallback 600s-timeout hardening** — `warren_buffett`'s fallback hit `OpenRouter … Upstream error from Nvidia: Decode wall clock timeout after 600s (502)`. Add a per-call timeout / prefer a more reliable fallback route so a dead fallback fails fast instead of blocking a persona for 10 min.
- [ ] **OpenBB free-provider data reliability** — on the AMZN run yfinance returned empty/insufficient data for a mega-cap (market cap $0, <20 price bars, null fundamentals) and news/insider providers (FMP/Benzinga/Intrinio) were unconfigured/restricted, starving personas → low-/zero-confidence signals. Diagnose whether yfinance is IP-blocked from the Oracle node (Yahoo blocks datacenter IPs); if so, switch the default equity/price/fundamentals provider to a free/keyless one that works from the node, or proxy yfinance. Paid FMP/Benzinga/Intrinio keys for news+insider are a separate cost decision.
- [ ] **Defensive tool-arg `limit` clamp** — the ben_graham fix is prompt-only; a wrapper that silently caps `limit` at the provider max would harden every persona against future prompt/model drift.
...
#### Sandbox
- [x] **Move Sandbox to a separate package: `langchain-opensandbox`** — shipped 2026-08-08 as [gururafiki/langchain-opensandbox](https://github.com/gururafiki/langchain-opensandbox) (MIT). Carries `OpenSandboxSandbox` (renamed from `OpenSandboxBackend` to match the `<Provider>Sandbox` convention of `E2BSandbox`/`ModalSandbox`/`DaytonaSandbox`), `OpenSandboxSettings`, the thread-scoped `SandboxFactory`/`get_backend`/`get_sandbox`/`aget_sandbox`, and `execute_python`. `muffin_agent/sandbox/` is now a shim re-exporting those plus the two store-bridge tools, which stay because they enforce `AccessControlledStore` namespace permissions. Not yet on PyPI — pinned by commit; publish + the LangChain `integration_external_docs.yaml` listing are the remaining steps.
...
- [x] **Migrate the sandbox off the sync client so `langgraph dev` no longer needs `--allow-blocking`** — done 2026-08-08 in `langchain-opensandbox`, flag removed from [.vscode/launch.json](.vscode/launch.json). The blocking call was not really in `SandboxFactory` but at deepagents' call site: `FilesystemMiddleware` resolves the backend on *every model call*, so an eager `get_backend` did a `list_sandbox_infos` + connect per call, synchronously, on the loop. `get_backend` now returns a `LazyOpenSandboxSandbox` that does no I/O at construction and serves `aexecute`/`aupload_files`/`adownload_files` from the async client (`BaseSandbox` derives `als`/`aread`/`awrite`/`aedit`/`agrep`/`aglob` from those three). A model call that never touches the sandbox now costs nothing.
- [x] **Fixed: `ls`/`glob`/`grep` silently returned nothing.** Found by running the real wire protocol against a live `opensandbox-server`, not by any test. execd emits one `OutputMessage` **per line with the newline stripped**; the backend joined them on `""`, so the newline-delimited JSON `BaseSandbox` parses collapsed into one unparseable blob — `ls` on any directory with 2+ entries returned `LsResult(error=None, entries=[])`, i.e. a wrong answer that reads as "empty directory". `execute_python` had its own copy and returned a two-line `print()` as one line. **The mocked fixtures embedded trailing newlines the real protocol never sends, which is exactly what hid it** — treat `tests/integration/_harness/sandbox.py` as proving muffin's wiring, never the provider's protocol.
