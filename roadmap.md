...
- [ ] Backfill E2E integration coverage for the deployable graphs still in `PENDING_INTEGRATION_COVERAGE` (`stock_evaluation`, `criteria_analysis`, `research`) — `council` and `trading_decision` are now COVERED. Move each into `COVERED_GRAPHS` as its `tests/integration/test_<id>.py` lands.
