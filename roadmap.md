...
- [x] **Tool telemetry (2026-07)** — `ToolTelemetryMiddleware` captures per-tool execution records (`tool_runs`) via a reducer-backed state channel (mirrors `subagent_transcript`; nested subagents merge up through the deepagents task tool). Off by default (`tool_telemetry_enabled`); the muffin-ui criteria page defaults it on and renders a per-criterion "Data collection" timeline + a run-level tool-execution summary. Records are reconstructed from message history in `aafter_agent`.
...
- [ ] **Consolidate the two capture middlewares (`subagent_transcript` + `tool_telemetry`) — they have duplicated purpose/mechanism.** Both walk `state["messages"]` in `aafter_agent`, both are reducer-backed parent/child channel pairs wired identically in `MuffinAgentBuilder._assemble_middleware`, and they duplicate helpers verbatim (`_running_as_subagent`, content-flatten `_text`/`_flatten`, `_capture`). `tool_runs` is fundamentally a **projection of the same message history** the transcript already captures — the only reason it can't be derived from `subagent_runs` today is that `SubagentTranscriptMiddleware._serialize` drops `ToolMessage.status` and `additional_kwargs` (cache metadata), which `tool_runs` needs. **Target design:** one `agent_capture` layer — a single message-walk per agent, a shared serializer that preserves ALL fields (add `status` + `additional_kwargs`), emitting the transcript as the source of truth and deriving `tool_runs` as a projection (backend or a UI selector over the enriched transcript). Must preserve two current behaviours the transcript alone doesn't: (a) telemetry captures **every** agent (incl. the parent stage agent's own `task`/direct calls) whereas the transcript is subagent-only for deep agents; (b) `tool_runs` rides the `values` stream (cheap, live) whereas `subagent_runs` for graph-node subgraphs needs the expensive `fetchSubagentRuns` history-walk (up to 15 `getState(subgraphs)` calls). Decide whether to keep both persisted channels behind one middleware or collapse to a single enriched channel + derived views. Context: this dedicated telemetry channel was shipped standalone (2026-07) with the duplication accepted as tech debt precisely so this consolidation is a follow-up, not a blocker.
