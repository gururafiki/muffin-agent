...
- [ ] **Tool telemetry for the other graphs** — `ToolTelemetryMiddleware` + the `tool_runs` UI ship for `criteria_analysis` first (config-gated via `tool_telemetry_enabled`); wire `research` / `trading_decision` / `stock_evaluation` / `investment` once the pattern is proven. Follow-ups: live telemetry streaming via a custom stream mode (`get_stream_writer()`), TTL/GC for leaked `("tool_telemetry", *)` store rows from crashed runs, and itemising individual `ToolRetryMiddleware` attempts rather than folding them into `duration_ms`.
