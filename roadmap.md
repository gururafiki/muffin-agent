...
- [ ] **Harden `agent_capture._running_as_subagent()` (deep-in-deep transcripts).** The builder wires `transcript_subagent_only=is_deep`, which is correct semantically: a deep agent invoked as a `CompiledSubAgent` of another deep agent DOES capture its transcript, because the guard is a RUNTIME check (`configurable.ls_agent_type == "subagent"`, stamped by the deepagents task tool) — locked by `test_guarded_capture_fires_for_deep_agent_as_subagent`. However, that check reads the ambient `get_config()` inside `aafter_agent` — the same mechanism suspected in the prod telemetry no-op. If it fails on the deployed runtime the guard fails CLOSED (nested deep agents silently capture no transcript; no duplicate-capture risk). No production graph nests deep agents via `task` today, so nothing is broken — but once the unconditional-capture deploy confirms (or refutes) the `get_config`-in-`aafter_agent` theory, either declare it safe or move subagent detection to a mechanism proven in prod (e.g. stamping a state flag from a hook where config is reliably available, like `awrap_tool_call`'s `request.runtime.config`).
